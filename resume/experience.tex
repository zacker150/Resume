%-------------------------------------------------------------------------------
%	SECTION TITLE
%-------------------------------------------------------------------------------
\cvsection{Experience}


%-------------------------------------------------------------------------------
%	CONTENT
%-------------------------------------------------------------------------------
\begin{cventries}
%---------------------------------------------------------

  \cventry
    {Senior Software Engineer} % Job title
    {Instabase Inc.} % Organization
    {San Francisco, CA} % Location
    {2026 - Present} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Led Development of secure code execution sandbox}
        \item {Led Development of SuperApp CI/CD on GitHub Actions, including infastructure provisioning via Terraform}
      \end{cvitems}
    }

  \cventry
    {Software Engineer} % Job title
    {Instabase Inc.} % Organization
    {San Francisco, CA} % Location
    {2022 - 2026} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Led Development of secure code execution sandbox}
        \item {Led Development of SuperApp CI/CD infastructure on GitHub Actions, including Terraform and }
        \item {Led Development of AI Hub billing system, including integration with Metronome and Stripe}
        \item {Developed support for Google Cloud Storage}
      \end{cvitems}
    }

%---------------------------------------------------------
  \cventry
    {Software Dev Engineer I Intern} % Job title
    {Amazon Inc.} % Organization
    {Seattle, WA} % Location
    {Jun. 2021 - Aug. 2021} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Designed and delivered prototype for new Alexa feature using Java Spring and Lombok in a continuous integration and deployment environment}
        \item {Wrote unit tests using JUnit and Mockito}
        \item {Identified and eliminated the root-cause of critical bugs blocking release to production}
        % \item {Deployed code in a continuous integration and deployment environment}
        % \item {Wrote }
      \end{cvitems}
    }
%---------------------------------------------------------
  \cventry
    {Researcher} % Job title
    {University of Houston} % Organization
    {Houston, TX} % Location
    {Jun. 2018 - Present} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Identified flaws in commonly-used security datasets, resulting in a paper in a top conference}
        \item {Designed a framework for benchmarking machine-learning based phishing detection methods using Python, resulting in two publications in top conferences}
        \item {Introduced Git and continuous integration to 10 person research group, resulting in improved collaboration and reproducibility}
        % \item {Wrote }
      \end{cvitems}
    }
%---------------------------------------------------------
  \cventry
    {Software Development Intern} % Job title
    {HP Inc.} % Organization
    {Houston TX} % Location
    {May. 2017 - May. 2018} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Designed and delivered new configuration software for Windows based thin clients using C\#}
        % \item {Wrote }
      \end{cvitems}
    }
\end{cventries}
